...
class GPOS_WooCommerce_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		$this->method_title         = __( 'Gurmehub', 'gurmepos' );
		$this->method_description   = __( 'Gurmehub - Çoklu ödeme çözümüleri', 'gurmepos' );
		$this->enabled              = true;
	...
	public function process_payment( $order_id ) {
		...
		$gateway           = gpos_gateway_accounts()->get_default_gateway();
		...
		$gateway
		->set_order_id( uniqid( "{$order_id}_" ) )
		...
		if ( $threed ) {
			$gateway->set_callback_url( home_url( "/wc-api/{$this->id}_callback" ) );
		} else {
			$gateway->set_payment_type( 'regular' );
		}
	...
	public function process_callback() {
		$gateway  = gpos_gateway_accounts()->get_default_gateway();
		$response = $gateway->process_callback( gpos_clean( $_REQUEST ) ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
	 ...
	 */
	public function success_process( GPOS_Gateway_Response $response ) {
	 ...
	 * @param GPOS_Gateway_Response $response Ödeme geçidi cevabı
	 */
	public function error_process( GPOS_Gateway_Response $response ) {
		$order = $this->get_order( $response->get_order_id() );
